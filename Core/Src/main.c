...
#define MPU_ADDR (0x68 << 1) // I2C addr
#define MPU_PWR_MGMT_1 0x6B  // PWR: PLL X
...
/* USER CODE BEGIN PM */
/* Runtime state */
static volatile uint8_t i2c_busy = 0;      /* I2C DMA active */
static volatile uint8_t data_ready = 0;    /* New sample */
static volatile uint8_t uart_tx_ready = 1; /* UART DMA idle */
static char uart_tx_buf[256];              /* UART TX buf */
/* 1 Hz pacing (TIM6 @200 Hz) */
static volatile uint16_t tick_count = 0;
static volatile uint8_t second_flag = 0; /* 1 Hz flag */
/* USER CODE END PM */
...
static float ax_g, ay_g, az_g, ax, az;
static float gx_dps, gy_dps, gz_dps, gy;
static float temp_c;
/* -------------------------------------------------------------------------- */
...
static float gyro_offset_y_dps = 0.0f;

/* PID gains */
static float Kp = 50.0f; /* tune */
static float Ki = 0.7f;  /* tune */
static float Kd = 0.0f;  /* tune */

/* Safety & scaling. */
static float safety_angle_deg = 50.0f;
static float max_power_scale = 0.8f;
static float min_power_scale = 0.6f;
...
/* I windup clamp */
static float integral_limit = 5000.0f; /* I clamp */

/* Complementary filter alpha */
static float cf_alpha = 0.95f; /* CF alpha */

/* Timing */
static const float dt = 0.005f; /* 1/200 Hz */

/* State (deg) */
static float pitch_acc_deg = 0.0f;
static float pitch_deg = 0.0f;
static float prev_pitch_deg = 0.0f;
static float gyro_pitch_rate_dps = 0.0f;
static float pid_P = 0.0f, pid_I = 0.0f, pid_D = 0.0f;
static float pid_u = 0.0f;
static uint8_t attitude_initialized = 0;

/* Loop time (us) */
static uint32_t loop_time_us = 0;

float duty;
/* Forward declarations */
static void control_update(void);
static void motor_set_output(float u_counts, float pitch_deg);
...
static void process_sample_and_stream(void)
{
  ...
  /* Apply offsets */
  ax = ax_g - accel_offset_x;
  az = az_g - accel_offset_z;
  gy = gy_dps - gyro_offset_y_dps;
...
static void transmit_once_per_second(void)
{
  ...
  int n = snprintf(uart_tx_buf, sizeof(uart_tx_buf),
                   "P= %.1f I= %.1f D= %.1f | Angle= %.2f PWM= %.0f | loop_us= %lu\r\n",
                   pid_P, pid_I, pid_D, pitch_deg, duty, (unsigned long)loop_time_us);
  if (n > 0)
  {
    uart_tx_ready = 0;
...
void HAL_I2C_ErrorCallback(I2C_HandleTypeDef *hi2c)
{
  if (hi2c == &hi2c1)
  {
    i2c_busy = 0; /* allow retry */
    /* Consider reinit on severe errors */
  }
}
...
#ifdef DWT_LAR
  DWT->LAR = 0xC5ACCE55; /* Unlock if needed */
#endif
  DWT->CYCCNT = 0;                     /* Reset */
...
/* Map pid_u to PWM+dir; safety cutoff */
...
static void control_update(void)
{
  ...
  /* Angle */
  pitch_acc_deg = atan2f(-ax, az) * 57.2957795131f; /* rad->deg */
  gyro_pitch_rate_dps = gy;                         /* dps */
  ...
  /* Comp filter */
  pitch_deg = cf_alpha * (prev_pitch_deg + gyro_pitch_rate_dps * dt) + (1.0f - cf_alpha) * pitch_acc_deg;
  prev_pitch_deg = pitch_deg;

  /* PID */
  float error = 8 - pitch_deg;
  pid_P = Kp * error;
  pid_D = Kd * (-gyro_pitch_rate_dps);
  pid_I += Ki * error * dt;
  ...
  loop_time_us = cycles_to_us(DWT->CYCCNT - start_cycles); /* loop us */
}
/* USER CODE END 4 */
